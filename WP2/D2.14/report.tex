...
% This would better fit elsewhere:
%
% All technologies developed in this work-package can be transferred
% to the European Open Science Cloud in fairly straightforward manner,
% as the communication protocol is HTTPS, and all data is written in
...
\texttt{Sphinx} documentation system. This offers very high quality output to
HTML and PDF, strong support for complex documents with
cross-references and deep structure, and smooth integration
...
Meanwhile, the community has explored several variants of those
workflows. On is the \texttt{jupyter-book}
project\footnote{\url{https://github.com/jupyter/jupyter-book}}, where
content is authored using markup and \Jupyter notebooks. The content
is then fed to the Jekyll static web site generator; just like the
\texttt{Sphinx} this system allows more control over the output
generated, but requires more time for development. Another is
PreTeXT\footnote{\url{https://pretextbook.org/}}, where documents are
authored in XML. Finally many authors in mathematics appreciate
writing their documents in LaTeX, using SageTeX to include code
snippets, although at this stage non PDF export is experimental.

Conclusion: we have demonstrated that the approach in
\delivref{dissem}{ibook1} and the notebook-based workflow (described
here) are complementary and each can be used dependent on the
situation. The notebook-based workflow fits well computation-based
topics or problem-oriented books where each section is relatively
short. It clearly has a lower barrier for authors; collaboration is
also easier, especially for people who are not excellent in ICT. For
complex books or documents where the strictly linear structure of
notebooks (e.g. there can't be a code cell within an itemize list)
becomes too rigid, it's worth using markup based solutions like
\texttt{Sphinx} or \texttt{PreTeXT} despite their higher complexity.

Transitioning from the notebook-based to the markup-based workflows
is relatively painless,
thanks to tools, partially developed in \ODK. We did so in
\delivref{dissem}{ibook1}: initial -- experimental -- versions of
materials were authored in notebooks and when they became mature we
converted them to \texttt{rst} format and included them in an
interactive book.

Whichever the authoring workflow is chosen, the document can be
automatically exported to a variety of format: PDF, interactive HTML pages
with interactive cells or collections of Jupyter Notebooks. It should
be observed that the latter two are similar. Which is better?
There is no clear-cut answer, but the key
...
Results and modification can be saved and shared in a single file.
With an interactive HTML page, the exploration can
be done on the spot with no download etc; however changes are limited
to code cells and cannot be easily saved; this also adds a dependency
on external services with their usual caveats in terms of privacy, e.g.
if sensitive data is to be treated.
We found that, for biophysics students, easy access to quick
experimentation with a single formula/equation/ODE is usefull, while
during a course, e.g. in computational fluid dynamics, working in one's
own notebook is the only practical option. We therefore recommend
to generate both HTML and notebook versions, with the former linking to
the latter.
