...
blue circel) by views we call \defemph{interviews} (see below). The MitM approach
...
project. In the middle we see the core MitM ontology (the blue clound) as an OMDoc/MMT
...
\item a \emph{knowledge representation subsystem} that provides data structures for the COMs and
  their properties.
\item a \emph{database component} that provides mass storage for objedts, and 
...
of the computational primitives of the respective systems.
