...
We have generated MitM interface theories for LMFDB elliptic curves by refactoring the
\LaTeX source of knowls into \sTeX (see Listing~\ref{stex-ec} for an excerpt), which can
be converted into flexiformal OMDoc/MMT automatically. The MMT system can already
type-check the definitions, avoiding circularity and ensuring some level of consistency in
their scope and make it browsable through \textsf{MathHub.info}, a project developed in
parallel to MMT to host such formalisations.
...
\emph{vs.} float \emph{vs.} integer tuple, for instance) and mathemtical types (elliptic
...
Based on these MimT interface theories we can generate I/O interfaces that translate
...
into the MitM architecture transparently. 
...
%%% TeX-master: "paper"
%%% End:
%  LocalWords:  flexiformal flexiformalization
