...
\ednote{TW: 
    TW: Recycle some existing text from somewhere, spin: MathWebSearch was an experimental tool that required a lot of work to setup per application
}

MathWebSearch \ednote{TW: cite} is a search engine enabling semantic formula search. 
For example, given a query like $a^2 + b^2 = c^2$\ednote{TW: Make formulae red} it will find documents containing $3^2 + 4^2 = 5^2$ or $x^2 + y^2 = z^2$.
It was originally developed by \ednote{TW:  TODO}. 

The MathWebSearch daemon takes in a set of Content MathML\ednote{TW: cite original}-encoded formulae.
...
For details, we refer the interested reader to \ednote{TW: cite}.  
...
\ednote{TW: Do we want to have a picture here?}
\begin{itemize}
    ...
    \item an \textbf{query input parser}\ednote{TW: Name?} which converts user input into a query the daemon can understand
    \item a \textbf{frontend} which sends user input to the query parser and daemon, and presents the query result to the user.  
\end{itemize}

\ednote{TW: 
    TW:    - ArXiV search - Zentralblatt search
}
