...
\ednote{Describe theory with symbols ternary MMTEval, binary Eval, nullary GAP, SageMath, etc.}

\ednote{Describe semantics of MMTEval, Eval}

\paragraph{Alignment-Based Transformation}

\ednote{describe how expressions are transformed}

\paragraph{System Design}

\ednote{describe MMT SCSCP server, connections with systems, processing of meta-language objects via transformation and external evaluation}

\ednote{reuse some of the text from the beginning of the section}

\paragraph{Calling from Individual Systems}

We have developed a Python module that allows calling the mediator from within \Sage.

\ednote{describe details of syntax in Sage}

\subsection{Examples}

\ednote{update syntax to the one described in the previous subsection}
