...
Figure~\ref{fig:cgtontology} shows some example alignments between symbols in the GAP content dictionary and the MitM ontology. The most difficult challenges here relate to the types of the symbols in the respective system API theories. GAP for example does not use a hard type system for its ontology. Furthermore, the exact number and types of arguments of various constructors (e.g. for polynomials or ring extensions) can differ slightly across systems. In these cases, the MitM interface theories used for translating content between systems need to be carefully chosen and aligned to function as intended.
