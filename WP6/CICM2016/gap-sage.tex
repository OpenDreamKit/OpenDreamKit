...
Given a mapping of corresponding methods in the systems, we can use the adapter pattern to
...
To get around this problem we have worked on a more semantic integration, where adapter
methods are made aware of the type hierarchies of the respective other system, see
Listing~\ref{lst:adapter} below. 
...
\texttt{Size} still holds if the MitM inter-views link the
...
In ongoing experiments, several variants of the annotations are tested to allow for adding annotations on existing
