...
Core to our goals is to make existing systems interoperable at a mathematical level. In particular, we have to establish a common meaning space that will 
...
expressing the meaning of mathematical concepts -- \defemph{mathematical vernacular}: the language of
...
blue circle) by views we call \defemph{inter-views} (interface views). 
\ednote{POD for MK. Both R1 and R3 had comments on the term "interview". One of them encouraged us to use "inter-view" if no better term could be found.}
The MitM approach
...
\item the \defemph{logic level} represents the languages we use for talking about the
  ...
  foundations. \ednote{R3: "views (the squiggly arrows)" - - what's going on with the 
