\section{Case Studies  (for \delivref{dksbases}{dkstheories})}\label{sec:cases}
...
% with open('sagetypes.json') as file:
% ...:     data = json.load(file)
% len({ axiom for d in data for axiom in d['axioms'] })
% 25
% len([d for d in data if d['type'] == 'Sage_Category'])
% 274
% sum([len({method for d in data for method in d[cls]['methods'].keys() }) for cls in ["element_class","parent_class","morphism_class"] ])
% 569
