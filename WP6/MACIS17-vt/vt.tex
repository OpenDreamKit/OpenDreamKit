...
\begin{figure}[ht]\centering
  ...
  \caption[Virtual Theory Architecture]{
    Virtual theory for \lmfdb elliptic curves (some declarations omitted for brevity) 
  }
  \label{fig:vtarch}
\end{figure}
...
The remaining theories (in blue) form the \lmfdb API CDs: the schema theory and the database theory, which we describe below.
...
Thus, it is not practical to fix a set of concrete API CDs.
Instead, the API CDs must be split into two parts: for each database table, we need
\begin{compactitem}
...
We list only two here as examples: the \textit{degree}, an integer, and the \textit{isogeny matrix}, a $3 \times 3$ matrix of integers.
