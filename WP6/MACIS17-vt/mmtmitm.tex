...
The work reported in this paper orginates from in the EU-funded OpenDreamKit
...
Figure~\ref{fit:mitmconnect} -- also modeled via \ommt theory graphs the \textbf{API
...
organized into an ``obejct-oriented'' inheritance structure via \textbf{inclusions} and
...
induced knowledge space, and given the represeted theory graph, the \mmt system can
...
\item the knowledge space corresponding to the data base content cannot be compressed by
  ``general mathematical priniples'' like inheritance. Indeed, redundant information is
...
From a system perspective, virtual theories are just like concrete theories, but without
the assumption of loading all declarations from a file on disk at system startup.  Instead
of loading all knowledge from an XML file, virtual theories load declarations in a lazy
fashion when they are required.  Here we do not even restrict ourselves to lazily reading
an XML file, on the contrary, in most use cases we actually create the \ommt
...
%  LocalWords:  tikzstyle withshadow draw,drop circle,fill 30,inner formalized
%  LocalWords:  MACIS17-interop
