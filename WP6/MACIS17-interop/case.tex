...
Jon, being a proficient \GAP user, also knows that he can now install a \emph{method} in \GAP that will compute the Galois group of any rational polynomial transparently for him whenever he calls \lstinline|GaloisGroup| for a rational polynomial in \GAP. \ednote{@MP: Can you just say how that method installation looks? I suspect that this is a one-liner, if so, I would like to see it here}
And thus the 300-line\ednote{MK@MP: please give the correct number} \lstinline|radiroot| package can be replaced by a one-line extension in GAP, taking advantage of the work of the SAGEMath community, and participating in any future improvements of \Sage. In fact Sage uses the PARI system -- another of the OpenDreamKit systems -- for the computation. 
