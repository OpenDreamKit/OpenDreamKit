...
Initially, Jane has already built in \Sage the ring $R=\mathbb{Z}[X_1,X_2,X_3,X_4]$, the group $G=D_4$, and the action $A$ of $G$ on $R$ that permutes the variables, and the polynomial $q = 3*X_1 + 2*X_2$.
She now calls \lstinline|MitM.Singular(MitM.Gap.orbit(G, A, q)).Ideal().Groebner().sage()|, which results in the following steps:
\begin{compactenum}
  ...
  \item The MitM server translates \lstinline|MitM.Gap.orbit(G, A, q)| to the \GAP system dialect and sends it to \GAP.
