...
Suppose Jane prefers working in \GAP, and she wants to compute the
...
(which has a well-established interface to \Pari,\ednote{MP: Is it good to say
  that this interface goes through \Sage right now?} which can be used to
...
this group is solvable.\ednote{MP: It is in fact also the Dihedral group on 4
  points}
...
\ednote{MP:alternate ending: They both compute with pari, so it's mighty
  pointless trying to compare results for testing...})
