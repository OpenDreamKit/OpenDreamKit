...
For example, the symbol $norm: Group \to ???$ declares the function that returns the normalizer of a group.
\ednote{@(MP,VV): clean this up and make sure it corresponds to the symbol names actually used in the ontology}
...
\ednote{@MP,VV clean this up, using the actual symbols of the GAP interface; make sure this relates to the example in the previous section}
...
\ednote{MK@NT: indeed this looks much better. We want to show an imaginary syntax that allows to conveniently delegate.}
\begin{lstlisting}
