...
the interaction layers of the respective systems.  Generally, mathematical software
systems apply operations on mathematical objects, which can be expressed as applications
of domain-specific constructors to primitive objects like numbers and other objects. The
results of these operations are again such objects. If we abstract from issues of surface
syntax -- which we can by assuming that the systems use a standard like
OpenMath~\cite{BusCapCar:2oms04}, the interface languages differ in their vocabularies --
the operations and constructors. The difficulty in the translation of objects and
functionalities between systems is to \emph{align} the respective vocabularies:
...
graph in the \OMMT Format~\cite{Kohlhase:OMDoc1.2,RabKoh:WSMSML13,uniformal:on}. For
each of the systems $A$ to $H$, we generate a theory graph \textbf{API Content
  ...
  ontology.}  in \OMMT which is aligned with the corresponding symbols in the MitM
ontology. As the API CDs and the MitM Ontology are both in \OMMT -- inside the cloud
in Figure~\ref{fig:mitm} -- we can make use of \textbf{\OMMT
...
To delegate a computation from $A$ to $B$, we need to translate $A$-objects into $B$-objects for the ``remote procedure call'' and vice versa for the results. This is where the \OMMT alignments come into play. 
Given a sufficiently complete set of MitM alignments we can translate between systems dialects using techniques from~\cite{MueRoYuRa:abtafs17}. This translation has been implemented in the \MMT system~\cite{Rabe:MAGMS13,uniformal:on}   -- an implementation of the \OMMT format and a set of mathematical knowlege management algorithms. 
...
The situation is best illustrated by (still partly fictional) example. Say Jane wishes to check the hypothesis that all abelian transitive groups are cyclic\footnote{This example is mathematically trivial; we have chosen it because it shows complex system interaction rather than mathematical plausibility; a realistic, mathematically motivated example will be discussed below.} using a MitM-based mathematical VRE.
...
\ednote{MK@NT: indeed this looks much better. We want to show an imaginary syntax that
  allows to conveniently delegate.}
\begin{lstlisting}
...
\end{wrapfigure}
At the \Sage level, the \lstinline|all| predicate \ednote{MK@NT: is ``all'' something that
already exists in \Sage? Does it behave like this? Please make sure that the text in this
