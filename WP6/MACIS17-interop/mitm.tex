...
As John is most familiar with the SageMath CAS, we expresses this operation as
\begin{lstlisting}
every ([g] scscp(gap|mmt,is_cyclic(g)),scscp(lmfdb|mmt,query("transitivegroup"))) 
\end{lstlisting}
\ednote{MK@NT: this is made up, can you make it more Sagey?}

\ednote{NT@MK: here is a first step; I am not sure yet in which form
  to introduce the mmt and scscp pieces. What exactly do we want to
  convey about them?}
