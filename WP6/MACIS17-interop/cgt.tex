...
\OMMT formalizations must be relative to a foundational logic.
These foundations are themselves formalized in \OMMT.
...
We furthermore follow the judgments-as-type paradigm by declaring a symbol \lstinline[mathescape]|$\vdash$:prop$\to$type| that maps propositions to the \defemph{type of their proofs}, which allows us to declare proof rules as functions mapping proofs (of the premises) to a proof (of the conclusion).
...
This lets us, e.g., define groups by a theory declaring the group operations and axioms and use \lstinline|group=ModelsOf group_theory| as the type of all models of said theory, i.e., the type of groups, as seen in Figure~\ref{fig:mitm1}.
...
Since axioms are turned into record type fields as well, actually constructing a record of type \lstinline|group| corresponds to proving that the record indeed forms a group.
...
The \textbf{abstract level} contains the theory of \emph{Groups}: the group
...
\OMMT allows elegantly expressing that two theories induce equivalent model types: We define group actions in two separate theories and use \emph{views} to express
their isomorphism.

At the \textbf{representation level}, abstract groups are represented as concrete objects
...
As a universal tool to bridge the gap between groups, we use representations and
canonical representatives are \emph{group homomorphisms}, particularly
embeddings and isomorphisms, which are also used extensively in \GAP.

At the \textbf{implementation level}, we encode implementation details: Permutation groups in \GAP
...
The computations happen at the \textbf{concrete level}: while the higher levels
...
through MitM, all \GAP operations are available for it, e.g., computing the size of the group and its isomorphism type.
