...
\begin{tikzpicture}[xscale=2, yscale=1.8]\normalsize
  ...
  \node (mh) at (2,1.3) {MathHub.info};
  \node[draw,database] (gl) at (2,.8) {GitLab};
  \node[draw] (m) at (2,0) {
    \begin{tabular}{c}
      MitM Server\\\hdashline MMT
    \end{tabular}
  };
  \node[draw,dashed,rounded corners=5pt,fit=(mh) (m),inner sep=5pt] {}; 
  \node[draw] (s) at (4,0) {
    \begin{tabular}{c}
  ...
  \node[draw] (c) at (3, -1.1) {
    ...
    \draw[dotted,->] (g) to[bend left=10] node[left] {\tiny API CDs} (gl); 
    \draw[dotted,->] (s) to[bend right=10]  node[right] {\tiny API CDs} (gl); 
