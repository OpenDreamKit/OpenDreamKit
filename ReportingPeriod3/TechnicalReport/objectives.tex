...
\item\label{objectives:core} \textbf{Core Components:}
  ...
  JupyterLab; see \longlocaltaskref{UI}{notebook-collab}.
...
\item \label{objective:community}
  ...
  in particular live collaboration features \longlocaltaskref{UI}{notebook-collab};
  ...
  see e.g. \longdelivref{dksbases}{persistent-memoization}.
  \end{itemize}
...
\item \label{objective:updates}
  ...
  \item RP3: The \SageMath and \GAP systems have been extended by a persistent memoization package, which allows to cache computational results and even share them between any system that implementes the memoization format; see \longdelivref{dksbases}{persistent-memoization} for details.
  \item Ongoing work on the MMT system which forms the basis of the
    \WPref{dksbases}: Work on \taskref{dksbases}{isabelle} in RP3 has led to a tight integration with the Isabelle theorem prover and a complete revamp of the code for indexing theory morphisms (crucial for the MitM-based integration of VRE components).
  \end{itemize}
...
\item \label{objective:sustainable}
  ...
  \item RP2, RP3: Migrating \Sage to Python 3, see \longlocaltaskref{component-architecture}{mod-packaging}.
  \end{itemize}
