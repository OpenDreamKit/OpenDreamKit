...
and is extended in \longmilestoneref{UI-vre} to more mature functionality.
...
and prototype support for live collaboration with \localtaskref{UI}{notebook-live-collab}.
...
have been delivered in earlier reporting periods
in the form of \delivref{UI}{ipython-kernels-basic} and later
...
Prototype components and plan for \delivref{UI}{jupyter-live-collab} had been delivered in the previous reporting period.
...
In addition, further releases of \textt{nbdime} from \delivref{UI}{jupyter-collab} have been made.
...
In this reporting period, nbval has received further activity and contributions and new releases in this reporting period.
...
Even though this reporting period contains no explicit deliverables for this task,
significant work has been done in the form of PEP (Python Enhancement Proposal) 590.
Python functions implemented in C are much faster than pure Python functions
but they have only limited support for introspection and documentation.
This is in particular a problem for Cython, which compiles Python-like code to C.
One solution is to create a custom class for Cython functions.
This works well for documentation, but has worse run-time performance
since custom classes use a slower calling protocol than functions implemented in C.
Now PEP 590 solves this by making available the fast calling protocol
to custom code. This way, there is no longer a performance loss when
using Cython functions. This has been implemented in the upcoming
Python~3.8 and Cython~3.0 releases. We expect not only Cython to benefit from this,
but also other similar projects like Pythran or Numba.
...
\TODO{nthierry - any further work on this to report?}
...
\TODO{kohlhase - any further work on this to report?}
...
\TODO{fangohr - anything new to report?}
...
\TODO{jdemeyer - anything to report this period?}
