...
\item RP3: \textbf{Google and OVH run binder instances as a free service for the community}
  \ednote{minrk: write one paragraph;
    this is an indicator of the adoption of Jupyter (not specifically of ODK)}
...
\item \textbf{Multi-Site involvement of Researchers (Mobility of Researchers)}\\
  ...
  \end{compactitem}
\item RP2 \textbf{Heavy interest by the theorem proving community about MitM Ontology}
  \ednote{@frabe: logipedia success story: add some date information; maybe update}
  \begin{compactitem}
...
\item RP3: \textbf{Immediate Community Building around \dmh}
  \begin{compactitem}
  \item On the Math Data Workshop in Cernay in August 2019  we invited two external
...
\item \textbf{\ODK based VRE deployments}\\
  \ednote{@minrk, @nthiery: update VRE deployments stats}
  We are
...
\item \textbf{Systems and languages integrated in Jupyter}\\
  \ednote{@minrk: update statistics on jupyter kernels,
    ideally with progression per year.}
...
\item \textbf{Use of Jupyter notebooks}\\
  \ednote{@minrk: update statistics on jupyter notebooks on GitHub,
    ideally with progression per year}
...
\item \textbf{Download statistics via platform-specific distribution
    channels}\\
  \ednote{@defeo: update download stats through deb, rpm, conda, ...}
...
\ednote{nthiery: story of Juliette's comic for binder spreading around}
...
\item \textbf{GAP packages: activity and adoption of best practices}\\
  ...
  past year.
  \ednote{@alex-konovalov: update GAP package statistics}
\item \textbf{SageMath packages on PyPI}\\
  ...
  \ednote{@defeo, @embray: update SageMath / pip package statistics}
...
\item \textbf{Systems made interoperable with the Math-in-the-Middle architecture}\\
  ...
  \item[data.mathhub.info] four new datasets with together about 9000 objects with (on
    average) 8 computed properties. Provenance sketches for all of them, one formalized in
    MMT. 
  \end{compactdesc}
...
\ednote{@defeo: update web site statistics,
  ideally with gradation per year or reporting period}
\begin{itemize}
...
\ednote{@IzabellaFaguet, @VivianePons: update workshop statistics}
...
\ednote{@IzabellaFaguet, @VivianePons: update diversity workshop success stories}

\ednote{@embray: success story for Sage Days @ Ibadan
  2-3 paragraph, with link to the blog post}
...
\ednote{@nthiery, @VivianePons: resolve redundancy between the list of courses here and in D2.17.
  Maybe mention here only courses not delivered by ODK as success
  stories + statistics on courses delivered by ODK members
  (number of students, levels, domains)
}
...
\ednote{@minrk: update impact of tools developed by OpenDreamKit}
