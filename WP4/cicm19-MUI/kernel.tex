...
\begin{newpart}{Example}
\paragraph{Example}

MMT uses theory graphs to model mathematical knowledge. 
A very simple graph, resulting in the model of a ring, can be found in Figure~\ref{fig:ringraph}. 
A theory graph consists of theories (each usually modelling a single concept) and relations between them. 
One such relation is the extension of a theory, called a structure. 

We start with a monoid theory on the bottom right. 
It consists of a set of elements \textit{U}, an operation \textit{op} and a unit \textit{unit}.
We then continue on the top right, by extenting the monoid into a commutative group (or \textit{cgp} for short). 
Afterwards we add the axiom of \textit{inv}ertability and the axiom of \textit{comm}utativity of the operation. 

Next we model a ring, using both of the previous theories.
We first use a monoid for the \textit{mul}tiplication operation, then a commutative group for the \textit{add}ition operation. 
Finalyy we only need to add the \textit{dist}ributivity axiom. 

Note that the theory graph never explicitly states all axioms required for a ring or a commutative group. 
Instead, it only uses structures to refer to the underlying concepts. 
One of the common uses for MMT is to flatten a theory graph into its' explicit form. 
This process if refered to as flattening. 
\end{newpart}
...
\begin{wrapfigure}r{8.5cm}%\vspace{-20pt}
