...
MMT\ednote{TW: Define or reference MMT} differs from typical computational engines in Jupyter in that it does not only (and not even primarily) perform computation but also handles symbolic expressions with uninterpreted function symbols, whose semantics is described by logical axioms.
...
\paragraph{Input}
\ednote{TW: re-state the fact that the Kernel more or less implements a REPL}
...
\ednote{TW: Forwards-reference to widgets}
\end{itemize}
...
\ednote{TW: Should be implemented properly}
\end{itemize}
...
\ednote{TW: Re-work this along with an updated system diagram}
...
After some experiments with different solutions (e.g., HTTP communication) \ednote{TW: we never did that} and discussion within the OpenDreamKit community, we identified the Py4J library~\cite{Py4J:on} as the best choice.
...
That makes our kernel very robust against \ednote{TW: what does this mean} and allows benefiting from future improvements to the MMT backend.
...
\ednote{FR: I've added this section}

To integrate Jupyter notebooks and MMT documents, we make use of two fortunate design properties:

Firstly, the storage format for Jupyter notebook, while not human-readable, is easy to reverse-engineer\ednote{TW: It's documented, reference nbformat}.
...
\ednote{TW: Think of an example; perhaps screenshots in here}
...
\ednote{TW: Redo with concrete cntent}
