...
\todo{There is some repeat in the last two sentences}
\todo{this web-based edition cycle is not quite about local editing
  support, right? Should it be moved to some other section?}
...
development of \sys, we aim for a courser\todo{? coarser?} granularity of integration: we integrate at the
...
breadcrumbs at the top localize it in the \textsf{sets} \todo{it's
  hard to tell that sets is in a different font and should be
  interpreted as a name and not a word} archive in the \textsf{SMGloM}
