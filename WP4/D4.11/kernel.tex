...
Therefore, we wrote a Python module (which is distributed with MMT) that performs the bureaucracy of aligning advanced Python and Scala features.\ednote{MK@KA: can we point to this in the MMT code?}
...
In this section, we provide a higher-level overview of how communication between Jupyter frontend, kernel and MMT-backend takes place.
...
\begin{newpart}{MK: just made this up from the screenshot (which I have cropped); move somewhere appropriate}
  ...
  \ednote{NT: presumably a brief overview of the conclusions of the previous deliverables would be useful.}
  Figure~\ref{fig:ac}\ednote{MK@KA: save much much more horizontal space (smaller browser window, so that it becomes believable that we embed this into a document.)} shows an example where Jupyter/MMT widgets are used for in-document computation as specified in D4.9~\cite{ODK-D4.9}.
  ...
  This design makes constructing such interfaces much easier and the interaction functionalities much more powerful: we indeed still have the full power of a computational kernel in Jupyter at our fingertips. 
