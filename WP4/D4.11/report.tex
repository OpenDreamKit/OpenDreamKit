...
% ask Tom about the server
% paper or report?
% show actual code? I suppose not
...
which currently runs on juypter.mathhub.info. %maybe show picture of it?
...
\section{Jupyter kernel for MMT} %isn't it more likely a MMT kernel for jupyter?
...
% I suppose we still want to be able to create theories etc.
% if we want to have other "commands" (e.g. active computation) 
% we will have to create a custom parser for that
...
% + the JupyterKernel Class on the Scala side? or does this go to the next section
...
% here we would have to think of a Syntax that we want to use 
...
% do we really want to explain the bridge here? if yes how detailed?
% explain what widgets are and how they work? if yes how detailed?
...
% To use the Python-Java Bridge we first have to enable the Py4JGateway extension on our MMT server


\section{Jupyter Notebooks on MathHub}
% write some demo notebooks stored in gl.mathhub.info
\subsection{special treatment when displaying notebooks in mathhub.info: "Runme button"}
% show them in jupyter.mathhub.info
