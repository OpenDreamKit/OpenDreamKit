\documentclass{standalone}
\usepackage{amsfonts}
\usepackage{tikz}
\usetikzlibrary{tikzmark}
\usetikzlibrary{mmt}
\usetikzlibrary{docicon}
\begin{document}
\def\cn#1{\mathsf{#1}}
\def\bT{\mathbb{T}}
\def\bL{\mathbb{L}}
\def\bM{\mathbb{M}}
\def\bE{\mathbb{E}}
\begin{tikzpicture}[xscale=2,yscale=2,remember picture]
  \tikzstyle{doc}=[draw,color=black,shape=document]

  \node[thy] (t) at (0,0) {$\mmtthy{time}{\bT:\cn{type}, s:\bT,h:\bT,\ldots}{}$};
  \node[thy] (l) at (2,0) {$\mmtthy{length}{\bL:\cn{type},m:\bL,\ldots}{}$};
  \node[thy] (m) at (4,0) {$\mmtthy{mass}{\subnode{cma}{\ensuremath\bM}:\cn{type},g:\bM,kg:\bM,\ldots}{}$};
  \node[thy] (e) at (2,1) {$\mmtthy{energy}{\subnode{cen}{\ensuremath\bE}:\cn{type},J:\bE,\ldots}{1J=1\frac{kg\cdot m^2}{s^2}}$};
  \node[thy] (c) at (0,1) {$\mmtthy{lightspeed}{\subnode{clv}{\ensuremath{c}}:\bL/\bT}{1c= 299 792 458\frac{m}{s^2}}$}; 
  \node[thy] (emc2) at (0,2) {$\mmtthy{EME}{E:\bE,m:\bM}{E=mc^2}$}; 
  \draw[include] (t) -- (e);
  \draw[include] (l) -- (e);
  \draw[include] (m) -- (e);
  \draw[include] (t) -- (c); 
  \draw[include] (l) -- (c); 
  \draw[include] (c) -- (emc2); 
  \draw[include] (e) -- (emc2); 

  \node[doc] (snip) at (4,1.5) 
  {\begin{minipage}{3.8cm}
      \[E=mc^2\] where \fbox{$E$ is the \subnode{den}{Energy}}, \fbox{$m$ is the \subnode{dma}{Mass}}, and
      \fbox{\subnode{cls}{$c$} the \subnode{dlv}{speed of light}}.
    \end{minipage}};
  \draw[dashed,->] (den) to[bend right=25] (cen);
  \draw[dashed,->] (dma) to[bend right=25] (cma); 
  \draw[dashed,->] (dlv) to[bend left=25] (clv); 
\end{tikzpicture}
\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
