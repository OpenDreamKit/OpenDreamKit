...
part in Figure~\ref{fig:activedocs}; see~\delivref{UI}{adstex} for an overview and comparison to
other \pn systems. The active document player is implemented in the MathHub
system~\cite{MathHub:on} -- see~\delivref{UI}{mathhub-editing} for an introduction in the context of the
...
standard~\cite{HHKLRAT:SCSCP10}; see~\delivref{component-architecture}{scscp-sage} for an account of the exact state of
...
technical details and~\cite{DehKohKon:iop16}, \delivref{dksbases}{design} for an account of the applications
