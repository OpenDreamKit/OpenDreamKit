...
\TODO{Do you foresee other hosted notebook servers than JupyterHub
  that could benefit from this implementation of collaborative live
  collaboration? E.g. could it potentially be used in Cocalc's, and
  maybe even supersede the native implementation?}

\TODO{Can the prerequisite on the server be very briefly described,
  here or below? }

\TODO{How strong is the condition of being hosted on the same notebook
  server? E.g. could we imagine reusing part or all of the current
  design for a ``appear.in/hubl.in'' like scenario, where two or more
  Jupyter users anywhere on the internet could use some lightweight
  central service as rendez-vous point to initiate a direct
  peer-to-peer synchronization between their Jupyter notebooks?}
...
\TODO{A CRDT implementation seems a useful thing per se, with
  applications to collaboration on many other types of documents. Can
  we foresee an impact of this implementation beyond the Jupyter
  ecosystem?}
