...
\end{itemize}
  ...
  tree, which is to big to fit in memory. After chasing around some bug on
  ...
  Since it's written purely in Python, the code doesn't well perform when the
  computation in each node is short. A good technology for this seems to be
  Cilk++, and we are currently doing experiment whose goal is to have a two
  stage parallel computation, where Cilk++ is doing the load balancing on a
