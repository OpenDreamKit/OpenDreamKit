...
This approach avoids the  swell in the bit-size of the intermediate computations and compute with the arithmetic precision of the output for a total cost of $\GO{n^5}$.
...
is sub-optimal. By better balancing the computation load between numerous arithmetic operation on small bit-size and
...
after each iteration, in order to detect stabilization and allow to terminate the loop earlier than what the possibly
...
Figure~\ref{fig:rnsdixon_parallel} illustrates how the number primes $\ell$
balances the workload between inversions and lifting. On a small instance (top
part), the overall work may increases for large $\ell$. This problem disappears
...
Graphic Processing Unit (GPU) have become a widespread component in modern high performance computing platforms. They
...
the GPU. This overhead is quadratic in the matrix dimension and thus becomes negligible compared to as the dimension
increases.
