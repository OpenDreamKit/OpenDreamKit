...
$k_\ell\ell\cdot\GO{n^2}$ and $k_\ell\cdot\GO{n^2\ell^{\omega-1}}$,
where $\ell$ is the parameter and $k_\ell\ell\approx n \cdot
\texttt{bitsize}$ is approximately a constant.
...
that by increasing $\ell$ we increase the potential for overall
parallelism. 
...
Now in parallel this is strikingly more interesting as shown in
...
no more ``red'' ), and the parallelization pays of ($9.5$ speed-up
with $16$ cores).
