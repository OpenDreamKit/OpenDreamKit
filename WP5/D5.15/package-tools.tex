...
The \GAP package \software{Example} by Werner Nickel, Greg Gamble and
...
%to GitHub, using \software{ReleaseTools/GitHubPagesForGAP}
%for new releases. The guidance for package authors, formerly offered
%as an appendix to the \software{Example} package manual, was moved to the GAP
%Reference manual in \GAP~4.9 release. Finally, the \software{Example} package
...
\software{AutoDoc} package \cite{autodoc} which creates
documentation from simple structured comments in the source code,
similar to \software{pydoc} or \software{javadoc}.

By now, \software{AutoDoc} has become very reliable and configurable, and
...
(e.g. the \href{https://homalg-project.github.io/}\software{homalg} project,
...
and was realized with the creation of \software{PackageManager}
...
\software{PackageManager} is a \GAP package itself, and is utilized in the
...
\end{verbatim}
    ...
    \end{mdframed}
    \caption{Installing a package using \software{PackageManager}.}
    \label{fig:pkgman-sample-b}
\end{figure}

The most important function provided by \software{PackageManager} is
...
centralized repository (as is common for the \software{APT} package manager) which
...
more liberal \software{pip} package manager for the Python language.
...
Over the course of \software{PackageManager}'s development, it has accumulated
...
\software{PackageManager} was written using modern open-source software technologies
...
\begin{table}[ht]
  ...
  \end{tabular}
  \vspace{0pt}
  \caption{Software engineering good practice checklist for \software{PackageManager}}
...
\begin{table}[ht]
  ...
  \end{tabular}
  \vspace{0pt}
  \caption{Dissemination good practice checklist for \software{PackageManager}}
...
their \GAP installations more easily.  The \cocalc system, for instance, now contains \software{PackageManager}
