...
a number of \GAP packages such as \software{MeatAxe64} and \software{Profiling}
...
%% \item
%% \software{datastructures} \cite{datastructures} by Markus Pfeiffer, Max Horn, 
...
\item
\software{Digraphs} by Jan De Beule, Julius Jonu\v{s}as, James Mitchell,
...
\item
\software{ferret} by Christopher Jefferson \cite{ferret} provides a C++
...
\item
\software{FinInG} by John Bamberg, Anton Betten, Philippe Cara, Jan De
...
\item
\software{MajoranaAlgebras} by Markus Pfeiffer and Madeleine Whybrow \cite{MajoranaAlgebras}
...
\HPCGAP and \software{MeatAxe64} methods have both been used to good effect in
this area.
%
% Possible demo?
%
...
\item
\software{matgrp}  by Alexander Hulpke \cite{matgrp} provides an interface
...
can exploit the power of \software{MeatAxe64} and/or \software{ferret}.

\item
\software{NormalizInterface} by Sebastian Gutsche, Max Horn and
...
\item
\software{SCSCP} by Alexander Konovalov and Steve Linton \cite{SCSCP}
...
for a short tutorial) makes it a suitable replacement for the \software{ParGAP}
package by Gene Cooperman, which has not been redistributed 
with \GAP since \GAP~4.9.

\item
\software{SemigroupViz} by Nathan Carter \cite{SemigroupViz} is an emerging
package which is based on the \software{JupyterViz} package \cite{JupyterViz}
...
\item
\software{walrus} by Markus Pfeiffer \cite{walrus} provides methods for proving 
...
\item
\software{YangBaxter} by Leandro Vendramin and Alexander Konovalov \cite{YangBaxter}
...
plan to use parallel computations with \software{SCSCP} and \HPCGAP.
