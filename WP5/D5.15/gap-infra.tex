...
\TODO{I'm wondering if this next paragraph really adds much - SL}
In addition to public infrastructure, we continue to use the Jenkins installation at USTAN 
for daily and weekly tests, running the package updates system (Subsection~\ref{pkg-update}), wrapping and
testing release candidates (Subsection~\ref{distro}) and other automated tasks. 
It is also very valuable for tests with larger runtime and memory requirements
...
\item as an alternative distributions (see subsection~\ref{distro}),
