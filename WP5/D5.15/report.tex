...
\TODO{more than 3000 citations by now: 2992 on web + 99 waiting for AK to update}
...
via SCSCP \cite{SCSCP}. We also report enhancements to the \GAP language and to
...
\TODO{clarify whether included in full or only pages for the recent releases}
...
\TODO{need to explain that the first public release of a package and inclusion for 
...
\TODO{Can we claim that the smaller number of minor releases (and the
  ...
  AK: said now there that we drop beta and concentrate on changes in master}
...
\TODO{Can we assemble some statistics to show the general progress of
  ...
  compare performance? AK: it is below, but maybe should to into regression testing?}
...
\section{GAP packages}

Other sections of this report described or briefly mention 
a number of \GAP packages that, were developed, published,
and eventually included (with several exceptions) into the
\GAP distribution over the reported period. In this Section
we briefly list some other notable packages that appeared
in the last several years.

\begin{itemize}

\item
{\sf datastructures} package \cite{datastructures} by Markus Pfeiffer, Max Horn, 
Christopher Jefferson and Steve Linton, which aims at 
providing standard datastructures, consolidating existing 
code and improving on it, in particular in view of \HPCGAP 
(added in GAP 4.9.3).

\item
{\sf Digraphs} package by Jan De Beule, Julius Jonu\v{s}as, James Mitchell, Michael Torpey and Wilf Wilson, which provides functionality to work with graphs, digraphs, and multidigraphs (added in GAP 4.8.2).

\item
{\sf ferret} package by Christopher Jefferson, which provides a C++ reimplementation of Jeffery Leon's Partition Backtrack framework for solving problems in permutation groups (submitted for the 
...
which constructs Majorana representations of finite groups (added in GAP 4.10.1).

\item
{\sf matgrp} package by Alexander Hulpke, which provides an interface to the solvable radical functionality for matrix groups, building on constructive recognition (added in GAP 4.8.2).

\item
{\sf NormalizInterface} package by Sebastian Gutsche, Max Horn and Christof S\"oger, which provides a GAP interface to Normaliz, enabling direct access to the complete functionality of Normaliz, such as computations in affine monoids, vector configurations, lattice polytopes, and rational cones
(added in GAP 4.8.2).
...
which provides a ``PackageWizard'' to fill in the details
of the intended package and create all the files needed for the start.

As required, \GAP packages should provide documentation. 
%It is possible to use several formats, one of them is obsolete, 
%originating from the deprecated 
...
One could create it using the same XML-based format that is used for the main \GAP manuals,
%and requires {\sf GAPDoc package} to build the documentation
but writing XML code by hand is very tedious and error-prone. In 2012 Sebastian Gutsche 
and Max Horn (presently both at the University of Siegen) started the 
{\sf AutoDoc} package \cite{autodoc} which 
allows to create documentation from source code comments
without writing XML files.
% and then automatically generates XML input for GAPDoc. 
By now, {\sf AutoDoc} became very reliable and configurable, and
...
Using Docker containers described in Section~\ref{gap-infra}
...
\TODO{find a place for the text below, copied from previous section}
