...
As mentioned, \GAP 4.10.0 (November 2018) was the first official \GAP release
to provide an API for 3rd party code to 
link \GAP as a library. It was based on the lib\GAP code developed
within \SageMath, 
but is intended to be more flexible and general-purpose.
...
descriptions). This has allowed \SageMath to drop its custom 
...
support safe multithreaded programming in \GAP, dubbed HPC-\GAP. This
has been in development for some time, and the HPC-\GAP and \GAP
...
different programme. This was an essential step to suppoder further development of 
HPC-\GAP, and ensure that new developments in \GAP could be quickly
incorporated.
It also provided users an opportunity to star to experiment 
with HPC-\GAP, which comes together with a new ``HPC-\GAP Reference Manual''.

\TODO{Explain what HPCGAP does for you, try and construct an example
  where it looks good. Decision to refactor to remove ward (still in
  progress). Something about challenges of huge legacy
  codebase. Maybe mention alkternative parallelisation options with
  SCSCP and IO_fork}
...
when a change caused an incorrect result, or a crash, or an error
...
not necessarily test interactions between them). The \GAP test
suite consists of a number of test files in the {\tt tst} directory,
...
use the similar approach to testing and provide their own regression tests.
For packages redistributed with \GAP we run the package update system, which
checks for their new versions, retrieves them, and then uses them in a
...
GAP~4 used regression tests since its inception, however, in former
times they were mostly run by developers manually, and a part of the
...
computers. Since 2009, \GAP team in St Andrews started to use Jenkins
(\url{https://jenkins.io/}) automation server to regularly run the 
...
Of course, \GAP is not new to the version control, and the former
