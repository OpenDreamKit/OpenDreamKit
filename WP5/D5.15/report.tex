...
In the end, the latter approach was taken for the following reaons: approved
package versions for a given release are likely to be included with the \GAP
installation anyway, and so it is unlikely that users would need to reinstall
these; it requires less additional infrastructure, since a
\texttt{currentPackageInfoURLList} is already maintained for testing purposes;
and it encourages active use, and therefore testing, of recent releases.
