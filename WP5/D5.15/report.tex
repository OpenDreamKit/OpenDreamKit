...
which adresses developments within the \GAP system to meet the general
objective of workpackage 5 (High Performance Mathematical
Computing), to ``improve the performance of the computational
components of OpenDreamKit'' on  a wide range of architectures, but
especially multi-processor systems.
...
over the period, but we will concentrate on, and links between
developments and \ODK goals.

\subsection{Context and Background}

\TODO{Remind them what \GAP is, how old it is, who uses it, how it has
  traditionally been used, etc.}

\TODO{Move on to the relatively recent past -- involvement with Sage,
  SCIEnce and \HPCGAP projects -- motivations for parallel/distributed
  support, what our users want and what equipment they have access to}

\TODO{Role of \GAP in \ODK, motivations for work on performance,
  robustness, composability, refactoring to support future development
  and maintenance, support for the growing package ecosystem}

\TODO{Structure of this report -- which we might need to rethink, we
  should maybe organise it more around motivations (performance,
  compatibility, robustness,....) and less around the components of
  the system. Also we should make some degree of reference to the
  task for which this is the deliverable. The major developments
  should be assigned to them if at all possible.}
...
\comment{I wonder if we should use ODK month numbers as well as, or
  ...
  and then perhaps should be explained in the preamble.}
...
(supporting threading) into
the core system some language extension  were backported to core
\GAP at this stage to make it easier to unify the codebases of \GAP 4 and \HPCGAP. 
...
software in our core system. Our caution is based on
our experience. Moving too quickly in this area risks the reliability and
...
also enabled us, as planned, to merge the \HPCGAP codebase  into the
...
\begin{itemize}
  \item a concise syntax for short
    functions with any number of arguments, allowing, for example, to write
    \verb|{a,b} -> a+b| instead of \verb|function(a,b) return a+b; end|. 
...
\TODO{Maybe put tables in Appendix}
