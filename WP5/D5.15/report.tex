...
\TODO{Make sure that there is sufficient materialin tghe introductionb
  and at the start of each section to guide the reader so they can
  decide which bits to pay attention to}.

\TODO{Review the ordering of sections and subsections}
...
\TODO{Difference between package release and redistrubution}
...
\subsection{Overview}
\TODO{Adjust to the final order}
...
\ref{package-tools} reports on new and enhanced packages relevant to the
...
\TODO{We should be consistent in terminology -- we publish releases
  rathert than releasing versions.}
...
\TODO{Sort out this para. I've moved semigroups to the front because
  it's an example that's actually doign cutting edge mathematics, but
  the text needs cleanup. Mention that semigroups is parallel in the C
  library. Is it used in \Sage via the Python bindings?}
...
to appear in the public GAP release SL: Agreed but not here. TODO
added above  }
...
\TODO{The below probably belongs with other material on regression testing}
...
\section{Improvements to \GAP Package Tools}\label{packages-tools}
...
\subsection{ReleaseTools and GitHubPagesForGAP}
...
\subsection{Continuous Integration Support for Package Authors}
