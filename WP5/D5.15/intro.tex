\TODO{Make sure that there is sufficient material in the introduction
  and at the start of each section to guide the reader so they can
  decide which bits to pay attention to}.

\TODO{Review the ordering of sections and subsections - reorder includes in report.tex as appropriate}
...
authorship. \comment{Mention package refereeing here?}
...
\TODO{Difference between package release (independently by its author)
and depositing it for the redistrubution with GAP}
...
via SCSCP package \cite{SCSCP}. We also report enhancements to the \GAP language and to
...
This report is divided into sections as follows: the next section (\ref{sec:core-gap})
reports relevant general developments in the core \GAP system. Section
\ref{sec:hpc} focuses on developments in the multi-threaded
system. Section \ref{sec:gap-infra} discusses our work aimed at improving
robustness and reliability, both of \GAP and its packages. This
includes both new tool support and new processes. Section
\ref{sec:package-tools} reports on new and enhanced packages relevant to the
...
since the last relevant deliverable of WP3. \comment{no separate 
Jupyter section at the moment}
