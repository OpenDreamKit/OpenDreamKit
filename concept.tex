...
\paragraph{The diversity of needs}

\TODO{Mathematicians have a variety of workflows, requiring each a
  different VRE.}

Mathematical research projects and teams that make of computation are
extremely diverse, in size, sophistication and requirements:
\begin{itemize}
\item At one extreme a project might consist of one researcher, with limited
...
\item At larger collaborative project might involve five or six researchers at two
...
\item Still another type of project would be a very large and open-ended
  ...
  we aim to support.
\end{itemize}
\TODO{Itemize the list of scenario, include Wolfram's comment about
  having a large array of tools under hand, VREs used by researchers
  in mathematics, but also used by engineers for instance as a tool,
  or by students sometimes.  So VREs needed w different level of
  complexity}

\TODO{Add more scenarios: mathematical collaborations and mathematical
  research projects involving collaboration also vary widely, as do
  the skills and needs of individual mathematicians, calling for a
  highly modular and customizable VRE infrastructure.  Some VREs need
  to support search, other computations.}

\TODO{Florent scenario : In a typical day of research in Algebraic
  combinatorics, we needs tools from very different areas of mathematics:
  linear, commutative and non-commutative algebra, symbolic manipulation,
  group theory, graph theory, language theory and rewriting techniques for
  examples... We need to write our own code that glue together the
  corresponding computing software in a novel way. Easy parallelization and
  distribution of the computations is required to cope with combinatorial
  explosion. Last but not the least, we need to visualize the results, which
  are often large graphs with complicated informations on the node or
  edges. Of course, we need to be able to exchange with colleagues not only
  the computed data but also the computation software.  }

\COMMENT{Specific challenges in maths}

\TODO{A specific challenge in mathematics comes from the vast yet
  tightly connected array of concepts involved. The natural ontologies
  of mathematics are richer, more complex, and more interconnected
  than for, say, fluid dynamics.}

\TODO{  Also: data and software in math not really patentable, no big money.
  No real ownership fights. 
}

\TODO{A scenario to illustrate the following point: The development of
  effective, flexible and easy-to-use VRE technology is especially
  important for mathematicians, because their research groups tend to
  be small and can be isolated.}
...
\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    ...
    will make it easy to set up Virtual Research Environments,
...
\paragraph{Benefit: meeting actual needs in Mathematics}
...
we are at a critical point providing an opportunity to do so.

\TODO{Faire un encadré avec les composants clefs: basic technologies:
  web, database, openstack ..., computational components, user
  interfaces}

\TODO{Key factor: development models and communities that want to
  collaborate}

The key factor is that open source development models and
collaborative tools (e.g. \texttt{github}) have reached a maturity
allowing for bringing together very large and diverse communities of
developers, and fostering large ecosystems of interoperable
components.

We will elaborate later in this section how this has specifically
affected the development of mathematical software in the last decade,
showcasing the sustainability of the ``by users for users``
development models even for general purpose mathematical computational
components. Simutaneously key open source technologies have recently
emerged: virtual machines and containers for easy deployment, cloud
infrastructure (open stack), web technologies (mathjax, WebGL, ...)
for powerful in-browser clients, scalable decentralized databases.
Combining this with the interactive computing environment
\Jupyter. \Jupyter (described later in this section), most of the
building blocks for constructing VREs are already there, and \SMC
proves the feasibility (but 70k of code).

Hence our approach is to join forces with \Jupyter and focus on
