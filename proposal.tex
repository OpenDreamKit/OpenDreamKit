...
- [ ] Concluding the brainstorming on the first paragraph: [WHO]
- [ ] Improving the little TODO's, proofread, improve transitions, ...: [WHO]
...
\COMMENT{In short: math is critical for innovation}
...
algorithms: Computer Assisted Tomography (CAT scanning) is based on solving mathematical
...
are an enabling technology for our modern world.
\end{draft}

\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    \COMMENT{In short: maths needs collaboration and computational tools}
    ...
    collaborative, geographically distributed team activity that
    supported by e-infrastructures. At the same time
    computational methods have become much more prominent in mathematical
    research, driven by the availability of computers and high-quality
    software as tools, and by interest in mathematical problems and
    proofs that cannot be addressed without massive computer
    calculations. \COMMENT{Possible examples, CoFSG and sequelae,
      Kepler conjecture, hardware verification.}

\end{boxedminipage}
\end{center}

The mathematics research community has led the uptake of many new
technologies, starting with electronic computers in the 1940s and
1950s. In 1945 Alan Turing wrote of his design for the NPL ACE computer, that

\begin{quote}There will be positively no internal alterations to be
  ...
  720.\end{quote}.  
More recently, on
the Web standards front, MathML was the first XML recommendation;
\url{planetmath.org} has been Web 2.0 even before Wikipedia; Computer
Algebra System research has been a driving force in the development of
HPC, and many now standard programming paradigms appeared originaly
there.


Software innovation by mathematical researchers continued in the
development, mainly in Europe of many highly successful, open-source,
community-developed specialized systems,  starting with \PariGP in
1979 and including \GAP in Group Theory, and
\Singular in Algebraic Geometry. This was at a time when most other
scientific computing research relied on bespoke Fortran programs
used for one calculation and then discarded. 
However for a long time those systems remained limited to specific
research topics and non-interoperable, and general purpose systems
supporting a broad range of engineering mathematicsm, arose in the
1980s and 1990s from
mostly US-based commercial companies like Wolfram
and MapleSoft, creating a profitable industry. At that time the
mathematical research community could not concentrate sufficient
manpower in to assemble a broad range of capabilities in a uniform 
computational framework and give it a
coherent mathematical user interface.

However, the last decade has witnessed the remarkable emergence of \Sage,
a truly general purpose computational mathematical system committed to
and drawing huge benefits from the power of open source software a
virtual software development environment. It's
wide success and acceptance showcased the viability of user-driven
community development models to deliver high quality software to
researchers, teachers, and practitioners in mathematics. A key factor
to the success and sustainability of \Sage is summarized in its motto:
``Build the car, don't reinvent the wheel''. \Sage is indeed built on
a large stack of existing open source software, ranging from the
aforementioned specialized computational systems, to \Python, a
general purpose programming language that is used by millions of
programmers worldwide. This architecture enables to focus on the
specific needs of the community while externalizing most of the
general purpose development efforts.

\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    \COMMENT{In short: maths needs VRE's}

    The next challenge is to deliver complete Virtual Research
    Environments, that support the entire life-cycle of computational
    work in mathematical research, from initial exploration to
    publication, teaching and outreach.

  \end{boxedminipage}
\end{center}

The first attempt in this direction to gain any real traction is 
the Virtual Research and Teaching
Environment \SMC (hosting more than 10k users and 100k projects after
just one year). There has also been considerable activity in a range
of online mathematical databases such as the Encyclopedia of Integers
Sequences, the Atlas of Group Representations and \LMFDB. These show
the community interest in well-executed \textbf{integrated solutions}
which can enable large-scale collaboration on Mathematical \textbf{software},
\textbf{knowledge}, and \textbf{data}.

In another direction systems such as polymath and mathoverflow show
the interest among mathematicians in exploring new forms of
collaboration, when the tools are well-designed and the balance of
effort and reward is correct.

At the moment setting up infrastructures such as these
requires massive ad-hoc efforts. \SMC required ?? lines of bespoke
code to integrate its components; each of the databases and
collaboration sites is essentially a bespoke program. The challenges
to a more flexible and resuable system include portability,
compatibility, performance, usability, reproducibility, and
understanding of the social dynamics of user and developer communities.
...
involving collaboration also vary very widely, as do the skills and
...
\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    ...
    \TheProject proposes to focus on delivering a \textbf{toolkit}
    that will make it easy to setup Virtual Research Environments,
...
whenever possible. A key technology here is \Jupyter, a set of
open-source software projects for interactive and exploratory
computing. These software projects help make scientific computing and
data science reproducible and multilanguage (\Python, Julia, R,
Haskell, etc.). The main application offered by Jupyter is the Jupyter
notebook, a web-based interactive computing platform that allows users
to create data- and code- driven narratives that combine live code,
equations, text, interactive dashboards and other rich media into a
single executable document. \Jupyter is already used very widely in
research and development, both in academia and industry and the user
base grows rapidly.
...
\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    ...
    (\LMFDB), and general purpose interactive computing components
    (\Jupyter).
  \end{boxedminipage}
\end{center}
...
\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    ...
    as well researchers, teachers, and practitioners in the
...
\TODO{
  ...
  \item Rather than making an opposition between academia and
    corporate world, maybe we could say that the work will be 
\end{compactenum}
}
...
\TODO{Make at least a token effort to reflect some dependencies and
