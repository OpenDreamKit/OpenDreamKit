...
\item\label{objectives:framework} To develop and standardise
...
\textbf{Objective \ref{objective:architecture}}
...
\TODO{\textbf{\Large What we would like the ``big picture'' in
    Figure~\ref{fig:thebigpicture} to highlight}
  \begin{description}
  \item[Human centered:] At the core: researchers and communities
    thereof.
  \item[Collaborative development of information:]
    Software, Knowledge, Data (currently in blue)
  \item[Physical resources:]
    (currently in red)
  \item[Flow of information and processes:]\ 
    \begin{description}
    \item[Software development] \emph{bug reports} and
      \emph{enhancement requests} emerge from the community, typically
      through collaborative help centers, and are posted on issue
      trackers. \emph{Design discussions} occur on mailing lists and
      issue trackers. Researchers \emph{submit code} to the code
      repositories. \emph{Quality control}: the code is reviewed and
      tested by continuous integration tools. Finally the code
      \emph{integrated} within computational components, and used by
      the community. Researchers (as well as other users: teachers,
      engineers, ...) interact at each step of the process.
    \item[Production of data] Computational components $\Longrightarrow$ databases
    \item[Scientific publication] researchers submit articles to journals and
      post them on preprint servers; the articles get reviewed by
      other researchers; finally they are distributed back to the
      community
    \item[Collaborative research]
      \begin{itemize}
      \item Subcommunities of researchers build their own
        collaborative work space (VRE?) by combining:
        \begin{itemize}
        ...
        \item Wikis-Knowledge bases (true for findstat, LMFDB): quicker cycle for consolidation of information spread over papers/brains
        \end{itemize}
      \item At this scale, they collaboratively develop software
        (e.g. specialized libraries), data and knowledge
        (e.g. articles).
      \item Whenever relevant, this information is contributed back to
        the community.
      \end{itemize}
    \end{description}
  \end{description}
