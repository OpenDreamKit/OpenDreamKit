...
\begin{abstract}
  ...
  \TheProject will deliver a flexible toolkit that will enable groups
  ...
  pure mathematics and applications. It will be built out of a sustainable
  ecosystem of community-developed open software, databases, and
  services, and using the available storage and computational
  resources.%
  %
  Components that can be combined in the environments include existing
  tools and their functionality such as \Sage, \GAP, and \Singular. We
  make use and extend the \Jupyter Notebook environment; and make a
  wide variety of important tools usable together.
  %
  %
  The environments will support seamlessly the full
  ...
  it and to work well on modern high-performance hardware. Interfaces
...
\TOWRITE{ALL The below does little more than repeat the table above
  it. Needs urgent work

  try to reflect some dependencies and
...
package~\WPref{component-architecture}, especially
with~\taskref{component-architecture}{interface-systems} which aims to
interface various softwares, and at the occasion of the community
building and dissemination activities of work package~\WPref{dissem}.

\paragraph{Work Programme for Objective 4: }

Individual pieces of software will be updated within work packages \WPref{component-architecture} and \WPref{hpc}.
...
This objective should also benefit from \WPref{social-aspects} (social aspects) with~\taskref{social-aspects}{isocial-decisionmaking}.
...
Objective 7 is a core part of \WPref{dksbases}, which deals with data
and its meaning.

\paragraph{Work Programme for Objective 8: }

We will demonstrate and explore the effectiveness of the VREs through
\WPref{dissem} (dissemination) and the study of the social aspects of
collaborative research in \WPref{social-aspects}.

\paragraph{Work Programme for Objective 9: }

The effective dissemination is achieved with part of~\WPref{dissem}. More
specifically, members of \TheProject will organise workshops with \taskref{dissem}{dissemination}
or \taskref{dissem}{project-intro}.
...
\gantttaskchart[draft,xscale=.3,yscale=.24,milestones] 
