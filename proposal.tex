...
\begin{draft}
Opening paragraph Option 1: (smoothing edits by Neil, integrating examples from below, hope that's the right spirit! Played down the separation a bit between maths and Engineering [don't want to upset anyone], but hope it's clear that maths is at the core.
\end{draft}
We live in an innovation-driven society. The key enabling tool for our
advances is mathematics. For example, the global positioning system
(GPS) needs relativistic mathematics, finite element calculations and mobile
phone connectivity depend on Delaunay triangulations, the modern
communications infrastructure is dependent on cryptographic algorithms
...
\begin{comment}
I think we need a punchy opening statement here to grab attention. Here is my attempt at it.
\end{comment}
...
massive ad-hoc efforts. \SMC required \TOWRITE{}{How many lines of
  code?} lines of bespoke code to integrate its components; each of
