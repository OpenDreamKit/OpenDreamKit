...
\begin{center}
  \begin{boxedminipage}{.95\textwidth}\em
    ...
    In the last three decades mathematics research has gone from a
    solitary pen-and-paper activity of talented individuals
    corresponding via lectures, letters, and journal articles to a
    collaborative, geographically distributed team activity that
    involves building joint research infrastructures. At the same time
    computational aspects have become much more prominent in math
...
The mathematics community was one of the first to take up new
technologies and has been in many cases driving them. For example, on
...
\TODO{reinsert the ``inovation'' keyword in the following paragraph}
In particular, soon emerged, in particular in Europe, many highly
successful open-source community-developped specialized systems like
\GAP in Group Theory, or \Singular in Algebraic Geometry, some early
as 1979 for \PariGP in Number Theory. However for a long time those
systems remained insular and non-interoperable, and general purpose
systems were the \TODO{monopoly} of mostly US-based commercial
companies like Wolfram and MapleSoft that made profits in the billions
in the last decades. It seemed only those had the developer power to
appropriate the innovations in a uniform computational framework and
give it a coherent mathematical user interface.

But then the last decade witnessed the remarkable emergence of \Sage,
a general purpose open-source computational mathematical system. It's
...
Indeed, the recent successes of the Virtual Research and Teaching
Environment \SMC (hosting more than 10k users and 100k projects after
just one year) and of the online number theory database \LMFDB indeed
showcase the strong needs for \textbf{integrated solutions} enabling
large-scale collaboration on Mathematical \textbf{software},
\textbf{knowledge}, and \textbf{data}.

Yet setting up infrastructures like the aforementioned currently
requires massive ad-hoc efforts. Challenges include portability,
compatibility, performance, usability, reproducibility, not to mention
the many social aspects involved in communities and ecosystems
thereof. A specific challenge in mathematics comes from the vast yet
tightly connected array of concepts involved. Different groups of
researchers may have radically different needs, workflows, and
resources, calling for a highly modular and customizable VRE
infrastructure.

\TODO{Better explain the specific needs in maths, and the necessity
  for VRE building blocks rather than VRE themselves}
