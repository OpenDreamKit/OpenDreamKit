...
\begin{wpdeliv}[id=mws,miles=startup,due=9,nature=OTHER,dissem=PU,lead=JU,issue=133, status=delivered]
  ...
  %   {Curated Math-in-the-Middle Ontology and Alignments for \GAP / \Sage / \LMFDB}
  % \end{wpdeliv}
\begin{wpdeliv}[due=42,miles=dksbases-interop2,id=persistent-memoization,dissem=PU,nature=OTHER,lead=SA,issue=143,status=submitted]
    {Shared persistent Memoisation Library for \Python/\Sage}
  \end{wpdeliv}
  \begin{wpdeliv}[id=nbad-search,due=48,nature=OTHER,dissem=PU,lead=FAU,issue=134,miles=dksbases-interop2,status=submitted]
