...
\begin{wpdeliv}[id=mws,miles=startup,due=9,nature=OTHER,dissem=PU,lead=JU,issue=133, status=submitted]
  ...
  \begin{wpdeliv}[due=24,id=conv,dissem=PU,nature=DEC,lead=ZH,issue=138,status=canceled]
    {Conversion of existing and new Databases (\LMFDB, OEIS, \FindStat) to unified interoperable
      System}
    merged into \localdelivref{interface-theories}.
  \end{wpdeliv}
  \begin{wpdeliv}[due=24,id=psfoundation,dissem=PU,nature=OTHER,lead=FAU,issue=139,status=canceled]
    ...
    merged into \localdelivref{interface-theories}.
  \end{wpdeliv}
  \begin{wpdeliv}[due=30,miles=WP6interop1,id=interface-theories,dissem=PU,nature=OTHER,lead=FAU,issue=247]
  ...
  \begin{wpdeliv}[id=notebooksearch,due=30,nature=OTHER,dissem=PU,lead=FAU,issue=140,status=canceled]
    ...
    merged into \localdelivref{fulltextsearch}
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,id=pssem,dissem=PU,nature=OTHER,lead=FAU,issue=141,status=canceled]
    ...
    merged into \localdelivref{mitmo}.
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,id=lfmverif,dissem=PU,nature=OTHER,lead=FAU,issue=142,status=canceled]
    {Curated Math-in-the-Middle Ontology covering exemplary parts of \LMFDB, OEIS,
      \FindStat, \GAP, \Sage knowledge bases.}
    merged into \localdelivref{mitmo}.
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,miles=WP6interop1,id=mitmo,dissem=PU,nature=OTHER,lead=FAU,issue=248]
{Curated Math-in-theMiddle Ontology and Alignments for \GAP / \Sage / \LMFDB}
\end{wpdeliv}
\begin{wpdeliv}[due=42,miles=WP6interop2,id=persistent-memoization,dissem=PU,nature=OTHER,lead=SA,issue=143]
    {Shared persistent Memoisation Library for \Python/\Sage} 
  \end{wpdeliv}
  \begin{wpdeliv}[id=nbad-search,due=42,nature=OTHER,dissem=PU,lead=FAU,issue=134,status=canceled]
    {Full-Text Search from Notebooks/Active Documents Interface} 
    merged into \localdelivref{fulltextsearch}
  \end{wpdeliv}
  \begin{wpdeliv}[id=fulltextsearch,miles=WP6interop2,due=42,nature=OTHER,dissem=PU,lead=FAU,issue=249]
