...
\begin{wpdelivs}
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=24,id=ipython-kernels,miles=startup,dissem=PU,nature=OTHER,lead=PS,issue=96,status=submitted]
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,miles=proto1,id=pari-python-lib2,dissem=PU,nature=OTHER,lead=UB,issue=84,status=submitted]
	  {Second version of the \Pari Python/Cython bindings}
  \end{wpdeliv}
    \begin{wpdeliv}[id=jupyter-import,due=36,miles=proto1,nature=DEM,dissem=PU,lead=FAU,issue=85,status=submitted]
      {Notebook Import into MathHub.info (interactive display)}
    \end{wpdeliv}

  \begin{wpdeliv}[due=36,id=vis3d,miles=UI-vre,dissem=PU,nature=OTHER,lead=SR,issue=86,status=submitted]
      {\Jupyter extension for 3D visualisation, demonstrated with computational fluid dynamics}
  \end{wpdeliv}
  \begin{wpdeliv}[due=24,miles=proto1,id=sage-sphinx,dissem=PU,nature=OTHER,lead=UG,issue=87,status=submitted]
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,miles=UI-vre-prototype,id=jupyter-live-collab,dissem=PU,nature=OTHER,lead=SR,issue=89,status=submitted]
      {Exploratory support for live notebook collaboration}
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,id=ipython-advanced-interacts,miles=UI-vre-prototype,dissem=PU,nature=DEM,lead=PS,issue=90,status=submitted]
