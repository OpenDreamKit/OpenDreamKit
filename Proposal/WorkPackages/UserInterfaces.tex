...
\begin{workpackage}[id=UI,wphases=0-48,
  ...
  USHRM=8, % Supporting reproducible data science and sharing of models
...
\begin{wpdelivs}
  \begin{wpdeliv}[due=18,id=pari-python-lib1,dissem=PU,nature=OTHER,lead=UB,issue=83, status=submitted]
	  {Python/Cython bindings for \Pari and its integration in Sage}
  \end{wpdeliv}
  \begin{wpdeliv}[id=adstex,due=9,miles=startup,nature=R,dissem=PU,lead=JU,issue=91, status=submitted]
  ...
  \end{wpdeliv}
    \begin{wpdeliv}[id=mathhub-editing,due=18,miles=startup,nature=DEM,dissem=PU,lead=FAU,issue=92, status=submitted]
      {Distributed, Collaborative, Versioned Editing of Active Documents in MathHub.info}
    \end{wpdeliv}
  \begin{wpdeliv}[due=12,miles=proto1,id=ipython-kernels-basic,dissem=PU,nature=OTHER,lead=PS,issue=93, status=submitted]
      {Basic \Jupyter interface for GAP, \PariGP, \Sage, Singular}
  \end{wpdeliv}
  \begin{wpdeliv}[due=12,id=ipython-kernel-sage,miles=startup,dissem=PU,nature=DEM,lead=PS,issue=94, status=submitted]
      {\Sage notebook / \Jupyter notebook convergence}
  \end{wpdeliv}
  \begin{wpdeliv}[due=12,id=jupyter-collab,miles=startup,dissem=PU,nature=OTHER,lead=SR,issue=95, status=submitted]
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[miles=community,id=cfd-vis,dissem=PU,nature=OTHER,lead=SR,issue=88,status=canceled]
      {Computational Fluid dynamics visualisation in web notebook}
      This deliverable was merged in \localdelivref{vis3d}.
  \end{wpdeliv}
