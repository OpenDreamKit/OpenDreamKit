...
Recall that ODK has several cross-cutting objectives, each of which
...
The management of intellectual property (IP) developed by a collaborative
distributed community over several decades by researchers with,
...
authors, and no transfers of IP rights were obtained, meaning that
many individuals and institutions undoubtedly own the IP of small
fragments of the system. This does not create a problem for continuing
distribution under the current license (since anyone distributing an
extended system implictly grants us permission to redistruibute their
...
all ?? million groups order up to 2000 (except order 1024). The
...
wholey on copyright rather than patents as the basis for its IP. After
discussion the \GAP community has taken the view that programs and
packages written in the \GAP language and smart documentes based on
the \GAP Jupyter kernel or similar technology (although requiring a copy of
\GAP  to run) are not derived works, and we make no attempt to control
...
\subsection{Dataset production and hosting}

\TODO{@kohlhase, @katjabercic: a couple paragraphs giving an
  indication on the volume of ODK's contribution around the production
  and hosting of datasets, with some assessment of its long term
  sustainability}

\subsection{ODK's web site}

\TODO{One paragraph about the sustainability of opendreamkit.org}
