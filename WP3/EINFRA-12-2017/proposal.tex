...
In particular, we propose integrating a service similar to mybinder
(mybinder.org), which provides publicly accessible Jupyter Notebook servers
based on the contents of GitHub repositories, which are a popular way of sharing
open source code and research materials. The existing service has been so
successful that it ran short of computational resources, proving that it has
identified just the right service for a critical need.
\\\hline
