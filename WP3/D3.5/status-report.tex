...
\TODO{Add links to the relevant trac tickets}

Task~\taskref{component-architecture}{workflow} of
\WPref{component-architecture} aims to improve the development
...
We noted that the existing command-line development tools for Sage
...
efforts to instead open Sage's development workflow up to the use of more
...
\item \url{GitLab.com} allows users to log in with their GitHub usernames, Google
    ...
    \TODO{Does this means that enabling GitLab login on Trac would
      indirectly enable logins with all those authentication
      providers?}
\item GitLab has some features that are not present on GitHub.  In particular,
    ...
    Task~\taskref{component-architecture}{workflow}
