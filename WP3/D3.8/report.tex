...
``health'' of the build \TODO{grammar in the preceding sentence is broken}.
...
enumerated in the following section\TODO{Should the preceding sentence be
negated?}.  Nevertheless, significant achievements were made by OpenDreamKit
...
cross-polination\TODO{cross-pollination?} on the subject.  Our experiences have
...
applies the proposed changes on top of it \TODO{This is not true for the CIs
that jrueth is familiar with. They'll not merge in master but just take what's
on that branch.} (if possible -- if not the build is rejected), and then
...
\TODO{Say something here about conda-forge? jrueth would be happy to say
something about conda-forge. What did embray have in mind?}
...
CI/build on non-free platforms: Windows, and ESPECIALLY macOS?  Or save that
for later...?}
