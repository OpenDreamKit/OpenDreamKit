...
exposed in Singular so far, in particular for since \texttt{FLINT} had to adopt
a different API than that of \Singular for representing multi-variate
polynomials.

As a consequence, exposing the new parallel multivariate polynomial code in
\SageMath would either require to wait for \Singular to adhere to \texttt{FLINT}
API, be released, and then integrated into \SageMath, or that the \SageMath
implementation be completely rewritten to directly rely on \texttt{FLINT},
bypassing \Singular.
Either way would require a significant development effort which was not possible
