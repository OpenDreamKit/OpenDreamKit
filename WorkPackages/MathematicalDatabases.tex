...
\begin{task}[title=Triform Theories in OMDoc/MMT,id=data-triform,
  ...
  The results are reported in \localtaskref{dkstheories} (report) \localtaskref{dksimp}
  (implementation).
\end{task}
...
\begin{task}[id=data-design,lead=JU,partners={ZH,US,SA,UW,LL},wphases={6-12,15-18!.33},PM=12,
  ...
  \localtaskref{design}. Conversion issues are covered in 
\end{task}
...
\begin{task}[title=FindStat Case Study (triformal theories),id=data-findstat,
  ...
  will be reported in \localtaskref{findstat}.
\end{task}
...
\begin{task}[title=\LMFDB Case study (triformal theories),id=data-LMFDB,
  ...
  \localtaskref{lmfmod}). Then we will perform cross-validation of the three model parts
  against each other (essentially model-based testing of software and inference; see
  \localdelivref{lfmverif}). Finally, we will pick an algorithm from the \LMFDB and verify
  ...
  results are reported in \localtaskref{lfmint}.
  \end{task}
...
\begin{task}[title=Memoization and production of new data,id=data-memo,
  ...
  Results are reported in \localtaskref{persistent-memoization}
