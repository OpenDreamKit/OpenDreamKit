...
\begin{task}[title={Formulation of requirements and design of new infrastructure when appropriate}, id=data-design]
  \TOWRITE{Michael/POD}{Rewrite database requirements design task -- done second pass, Michael have a look?}

Ontologies are the canonical method used to implement databases that require significant data interchange. However, because of the extreme reification present in mathematics (relations between objects themselves become objects of study), there are specific obstacles compared to the usual semantic web model of publishing.  

Drawing on semantic web/Linked Open Data experience of the Logilab group, specialised to mathematics through the OMDoc/MMT work of the Bremen group, we will design a decentralised infrastructure for \TheProject. This infrastructure would allow modular collaborations, through decentralised hosting of data without the need to merge everything centrally. 

We will organise a workshop associated to this task.

\TODO{original comment: provide more info about above claim: ontologie not suitable because of
  extreme reification}

\TODO{original comment: Logilab suggests to organize workshop first, to confront different
  cultures before comitting to the design of a new infrastructure. For example,
  based on its experience with CubicWeb, Logilab would promote the decentralized
  mechanism of the (semantic) web for publishing mathematical data, knowledge
  and software.}

% TODO: logilab participation to workshop and collaborative comparison: PM=2

% Building on the decentralized foundation of the internet, each university, lab
% and research group could publish its own data while following the principles
% of Linked Open Data and some tools would later harvest the different sites to
% index all the available resources. This would facilitate the discovery of data
% knowledge and software relevant to a particular project for a user, without
% requiring to merge everything in the same place a priori.
