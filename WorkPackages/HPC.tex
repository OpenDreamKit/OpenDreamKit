...
\begin{wpdescription}
  ...
  In Tasks~\localtaskref{hpc-gap}, and \localtaskref{pythran} (deliverables 
\delivref{hpc}{pythran-cython}
  and~\delivref{hpc}{pythran-sage}), we will also boost performance by
...
\begin{task}[title=Singular,lead=UK, PM=84, id=hpc-singular,partners={SA,UK} ]
...
\begin{task}[title=HPC infrastructure for combinatorics,id=hpc-combi,PM=6,lead=PS,partners={PS,UB}]
...
\begin{task}[title=Pythran,id=pythran,lead=LL,partners={UJF,LL},PM=24]
...
\begin{task}[title=Sun Grid Engine Integration in Project Jupyter Hub, lead=USH,id=hpc-jupyter,PM=12,partners=USH]
...
\end{wpdeliv}
  \begin{wpdeliv}[due=24,id=HPCcombi,dissem=PU,nature=DEM,lead=PS]
...
\end{wpdeliv}
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=36,id=HPCcombi,dissem=PU,nature=DEM,lead=PS]
