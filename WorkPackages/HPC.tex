...
\begin{WPDeliverables}
  \WPDeliverable{3}{del:QS_sieving}{Parallelise the relation sieveing component
of the Quadratic Sieve}
  \WPDeliverable{9}{del:QS_linalg}{Implement a parallel version of 
Block-Wiederman linear algebra over GF2 and the triple large prime variant.}
  \WPDeliverable{12}{del:FFT}{Take advantage of multiple cores in the matrix
Fourier Algorithm component of the FFT for integer and polynomial arithmetic,
and include assembly primitives for SIMD processor instructions (AVX, Knight's
Bridge, etc.), especially in the FFT butterflies}
  \WPDeliverable{12}{del:singular_polymul}{Parallelise the Singular sparse
polynomial multiplication algorithms}
  \WPDeliverable{12}{del:singular_polyarith}{Parallel versions of the Singular
sparse polynomial division and GCD algorithms.}
\end{WPDeliverables}
\end{task}
...
\begin{WPDeliverables}
  \WPDeliverable{12}{del:MPIRsuperoptimiser}{Extend the existing assembly
superoptimiser for AVX and upcoming Intel processor extensions}
  \WPDeliverable{24}{del:MPIRprocessors}{Ongoing support of Intel, AMD, ARM, 
Sparc processors and new Operating System versions}
  \WPDeliverable{12}{del:MPIRfat}{Provide FAT binary support for all new
x86\_64 processors, build system maintenance and improvements to tuning,
profiling and testing subsystems}
\end{WPDeliverables}
\end{task}
...
\begin{WPDeliverables}
  \WPDeliverable{3}{del:HPCcombi}{Turn the Python prototypes for tree exploration into
    production code, integrate to \Sage.}
  \WPDeliverable{12}{del:HPCcombi}{Explore the possibility to
    interface smoothly \Pythran, \Cython and \texttt{Cilk++}.}
  \WPDeliverable{24}{del:HPCcombi}{Refactor the existing combinatorics \Sage
    code using the new developed  \Pythran and \Cython features.}
\end{WPDeliverables}
\end{task}
