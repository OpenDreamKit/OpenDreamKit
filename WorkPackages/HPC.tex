...
\end{wpdescription}
\begin{tasklist}
\begin{task}[title=PARI,id=hpc-pari,PM=12, lead=UB, partners=UB]
...
\TODO{Say more, batch schedulers, authentication for
...
\begin{task}[title=HPC infrastructure for combinatorics,id=hpc-combi,PM=4,lead=PS,partners=PS]
...
\begin{wpdelivs}
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=3,id=QS-sieving,dissem=PU,nature=DEM,lead=UB]
      {Parallelise the relation sieving component of the Quadratic Sieve}
  \end{wpdeliv}
  \begin{wpdeliv}[due=3,id=HPCcombi,dissem=PU,nature=DEM,lead=UPS]
...
\end{wpdeliv}
  ...
  \end{wpdeliv}
  \begin{wpdeliv}[due=9,id=QS-linalg,dissem=PU,nature=DEM,lead=UB]
