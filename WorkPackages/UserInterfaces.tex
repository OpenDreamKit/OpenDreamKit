...
\begin{task}[id=notebook-collab,title=Notebook improvements for collaboration,lead=SR, partners={PS,USH,JU,USO}]
  ...
  version control workflows \localdelivref{jupyter-collab}.
...
\begin{task}[title=Structured documents,id=structdocs,
  ...
  that meets the expectations (see \localdelivref{adstex}). The implementation will be done
  through an iterative process that tries to enhance little-by-little the already existing
  softwares and make them interoperable and synergistic.  Results of this convergence will
  be reported in~\localdelivref{ipython-kernel-sage} and \localdelivref{jupyter-import}
\end{task}
