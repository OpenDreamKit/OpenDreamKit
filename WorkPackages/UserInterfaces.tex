...
\begin{task}[lead=UB,title=Python/Cython bindings for Pari,PM=16,id=pari-python,wphases=0-6,24-36]
  ...
  and integrate it in \Sage (\localdelivref{pari-python-lib1}). Secondly, different optimization
  will be provided for a tighter communication between \Pari and \Python.
  \begin{compactitem}
  \item Use the declaration files of \software{GP} to automatize \Cython declarations.
  \item Replace copy from the \Pari stack by direct pointers.
  \item More tests and documentation.
  \item Integrate the parallelization features from \taskref{hpc}{hpc-pari} within \Python.
  \item Implement a crossed bug report system between \Sage and \Pari (using
  results of \taskref{social-aspects}{social-decisionmaking}).
  \end{compactitem}
...
\begin{wpdelivs}
  ...
  \end{wpdeliv}
    ...
    \end{wpdeliv}
  \begin{wpdeliv}[due=36,miles=protol,id=pari-python-lib2,dissem=PU,nature=OTHER,lead=UB]
