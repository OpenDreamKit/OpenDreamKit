...
\begin{task}{OOMMF case study: Create Python interface to OOMMF code}
  ...
  maintenable with minimal effort while the OOMMF core code is
  developed further by the OOMMF authors.

  The interface will expose the raw C++ objects in Python, and for
  clarity we will refer to this interface as \texttt{OOMMF-py-raw}, to annotate
  that this gives access to OOMMF from PYthon but in a RAW
  way. Creation of this \texttt{OOMMF-py-raw} is technically doable as OOMMF
  had been written allowing to do this from Tcl.

  \textbf{Task B}: The \texttt{OOMMF-py-raw} library for Python provides access to the
  OOMMF functionality but requires some care from the user. In this
  second task, we will create a user friendly Python library that
  ...
  the well-received Nmag simulation package \cite{Nmag-url}.
  ...
  the micromagnetic standard problem 3 \cite{Micromagnetic-Standardproblem-3}, traditionally multiple
  OOMMF simulation runs would have to be conducted, and for each of
  those a new configuration file as to be written. Between these the
  size of the simulated geometry needs to be modified until two
  particular values of energy are the same. Given the new interface
  developed in this work package, this whole process can be replaced
  by one Python script that creates multiple OOMMF simulations,
  combined with a root finding method for the automatic iterative
  determination of the required simulation geometry.
  ...
  OOMMF-py, documentation and tests are created simultaneously with
  the code.
...
\begin{WPDeliverables}
  \WPDeliverable{15}{del:oommf-nb}{IPython Notebook Interface
    for OOMMF (\texttt{OOMMF-NB})}
\end{WPDeliverables}



\begin{task}{OOMMF case study: \texttt{OOMMF-NB} tutorial and documentation}
  % 6 person months
  ...
  \texttt{OOMMF-NB}, combined with complete documentation of the
...
\begin{WPDeliverables}
  \WPDeliverable{21}{del:oommf-nb-documentation}{\texttt{OOMMF-NB}
    executable tutorial and documentation}
\end{WPDeliverables}


\begin{task}{OOMMF case study: \texttt{OOMMF-NB} virtual environments}
  % 3 person months
  ...
  but for a server that provides the \texttt{OOMMF-NB} software and \texttt{OOMMF-NB}
  ...
  environments underpin the web hosted temporary \texttt{OOMMF-NB}
  ...
  [XXX]). Setup and maintenance of this machine is part of this work
  task.

\end{task}

\begin{WPDeliverables}
  \WPDeliverable{24}{del:oommf-nb-tmp}{\texttt{OOMMF-NB}
dynamic web service available}
  \WPDeliverable{24}{del:oommf-nb-virtual}{\texttt{OOMMF-NB}
...
\begin{task}{OOMMF case study: Evaluation and refinement}
  % 4 person months
  We will make the \texttt{OOMMF-NB} open source code available to
  ...
  \citett{OOMMF-NB} enables new and better science, (ii) where it
...
\begin{WPDeliverables}
  \WPDeliverable{48}{del:oommf-nb-evaluation}{\texttt{OOMMF-NB}
evaluation report}
\end{WPDeliverables}
