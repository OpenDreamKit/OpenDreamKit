...
\begin{task}[title=Dissemination and Communication activities, id=dissemination-communication]
  ...
  \TODO{neil: this coming year i will do four workshops, although the
...
days) on \TheProject for researchers and lecturers in all
disciplines. At Sheffield, this could be added to the set of courses
that are offered as part of IT Services' research support
department. As such, it could potentially reach all disciplines.
...
notes \delivref{dissem}{ils-}. The focus for the student here will not necessarily be on
...
\begin{compactitem}
\item How to create a monograph at reusing independently working
  building block of text and code?
\item When the interactive worksheet should be used and when
  executable code cell inside interactive textbook is sufficient?
\item What are best tools and practices for using single source for
  with many output targets?
...
\begin{wpdelivs}
 ...
 \begin{wpdeliv}[due=36,id=ils-lecture,dissem=PU,nature=DEM]{Demo: Interactive lecture notes and marking systems based on \TheProject.}\end{wpdeliv}
