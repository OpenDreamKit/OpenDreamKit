...
\begin{task}[title=Dissemination and Communication activities, lead=PS, id=dissemination-communication]
...
Apart from this different events, we will also be present at important events of both our scientific community (international mathematical conferences such as FPSAC for combinatorics) and the python / open-source software development community: PyCon, SciPy, EuroPython, etc. Two reports will be delivered throughout the project on the impact of all the mentioned actions (\delivref{dissem}{dissem-1}, \delivref{dissem}{dissem-2}).
...
id=training-portal,lead=PS,PM=2] 
Training is at the heart of this project: through our open source
approach, networking activities, workshops, demonstrators
(\localtaskref{index-librorum-salvificorum}, interactive books
(\taskref{UI}{oommf-nb-documentation}, \localtaskref{ibook}, and
training for teachers and trainers (\localtaskref{project-intro}), we
have firmly integrated the training aspect into our project plan. 

Each of these activities will created dedicated webpages hosting the
...
serves as a single starting point, gathering links to all training
activities and documents available. We will use standard community
building technology such as mailing lists, Wikis, Forums, Twitter
feeds, github to support this.
\end{task}
\end{tasklist}
